...
def test_parse_request():
    request = "foo 2 3 4"
    func, args = pulseforge.server.parse_request(request)
    assert func == "foo"
    assert args == ["3", "4"]


@patch("socket.socket")
@patch("pulseforge.server.send_file_to_secondary_server")
def test_handle_client(mock_send_file, mock_socket, mock_plugins, tmpdir, caplog):
    ...
    config_file_content = {
        ...
        "secondary_server": {"url": "http://localhost:8000/upload"},
        "plugin_dir": str(tmpdir.mkdir("plugins")),
        "log_dir": str(tmpdir.mkdir("logs")),
        "client": {"host": "localhost", "port": 65431},
    }
...
"""
    ...
    main_log_file = log_dir.join(f"session_{pulseforge.server.session_start_time}.txt")
    caplog.set_level(logging.DEBUG)

    with caplog.at_level(logging.DEBUG):
        with patch("pulseforge.server.load_plugins", return_value=mock_plugins):
            with patch(
                "logging.FileHandler",
                lambda filename: logging.FileHandler(str(log_dir.join(filename))),
            ):
                pulseforge.server.start_server()  # Starts the server and handles the mock client connection
                ...
                function_log_file = os.path.join(
                    log_dir, f"foo_{pulseforge.server.session_start_time}.txt"
                )
...
@patch("socket.socket")
@patch("pulseforge.server.send_file_to_secondary_server")
def test_handle_client_function_not_found(
    mock_send_file, mock_socket, mock_plugins, tmpdir, caplog
):
    # Prepare the mock socket
    mock_conn = MagicMock()
    mock_socket.return_value.accept.return_value = (mock_conn, ("127.0.0.1", 12345))
    ...
    log_dir = config_file_content["log_dir"]
    os.environ["LOG_DIR"] = log_dir
    main_log_file = os.path.join(log_dir, f"session_{pulseforge.server.session_start_time}.txt")
    caplog.set_level(logging.DEBUG)
...
@patch("socket.socket")
@patch("pulseforge.server.send_file_to_secondary_server")
def test_handle_client_invalid_request(
    mock_send_file, mock_socket, mock_plugins, tmpdir, caplog
):
    # Prepare the mock socket
    mock_conn = MagicMock()
    ...
    with caplog.at_level(logging.DEBUG):
        with patch("pulseforge.server.load_plugins", return_value=mock_plugins):
            with patch(
                ...
                assert any(
                    "Error" in str(call) for call in mock_conn.sendall.call_args_list
                )
                # Verify logs
