...
if CONFIG_FILE_PATH is not None:
    ...
    config = {}
    config["client"] = {"host": None, "port": None}
    config["server"] = {"host": None, "port": None}
    config["secondary_server"] = {"url": None}
...
PKG_DIR = pathlib.Path(os.path.abspath(__file__)).parents[1].resolve()
...
PLUGIN_DIR = config.get("plugin_dir", PLUGIN_DIR)
...
def setup_function_logger(function_name):
    ...
    function_log_filename = os.path.join(
        LOG_DIR, f"{function_name}_{function_start_time}.txt"
    )
        ...
        if function_name in plugins:
            ...
            if func:
                ...
                function_logger.info(
                    f"Function {function_name} called with args: {args}"
                )
                ...
                function_logger.info(
                    f"Function {function_name} generated file: {file_path}"
                )
