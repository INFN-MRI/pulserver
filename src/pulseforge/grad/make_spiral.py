...
def make_spiral(
    ...
    fov /= 10.0 # [mm] -> [cm]
    max_grad = pp.convert.convert(max_grad, from_unit="Hz/m", to_unit="mT/m", gamma=system.gamma) / 10
    max_slew = pp.convert.convert(max_slew, from_unit="Hz/m/s", to_unit="T/m/s", gamma=system.gamma) * 100
    ...
    if fs_npix is None:
        g0, k, _, _, dens = pg.spiral_varden(fov, res, grad_raster_time, fufa * max_slew, fufa * max_grad, 0.0, 0.0, narms)
        g, _, _, _, _ = pg.spiral_varden(fov, res, grad_raster_time, fufa * max_slew, fufa * max_grad, 0.0, 0.0, narms, rewinder=True)
    else:
        fres = fov / fs_npix
        gf, _, _, _, _ = pg.spiral_varden(fov, fres, grad_raster_time, fufa * max_slew, fufa * max_grad, 0.0, 0.0, 1)
        ...
        # actual calculation
        g0, k, _, _, dens = pg.spiral_varden(fov, res, grad_raster_time, fufa * max_slew, fufa * max_grad, densamp, denstrans, narms)
        g, _, _, _, _ = pg.spiral_varden(fov, res, grad_raster_time, fufa * max_slew, fufa * max_grad, densamp, denstrans, narms, rewinder=True)
